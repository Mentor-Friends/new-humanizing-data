...
//   console.log(categoryNameList,"namelist");

//   categoryListHTML = categoryNameList
//     ?.map((category: any) => {
//       // const isSelected = category?.data?.the_name === "Jobs" ? "selected" : "";
...
export async function submitAddItemForm(e: any) {
  ...
  console.log(platformList, "platformsss");

  // Check for "Humanizing Data" in the platformList
  const filteredItem = platformList.find(
    (item: { data: { the_listing: string } }) =>
      item.data.the_listing === "Humanizing Data"
  );
  if (filteredItem) {
    console.log(filteredItem.id, "id for Humanizing Data");
  }
  ...
  try {
    ...
    const itemId = itemConceptResponse?.id;
    console.log(itemId, "itemssId");
    ...
    if (itemConceptResponse) {
      if (filteredItem) {
        try {
          const Concept = await GetTheConceptLocal(itemId);

          const Platform = await GetTheConceptLocal(filteredItem.id);

          // Create the connection between the item and the humanizing data
          await CreateConnectionBetweenEntityLocal(
            Concept,
            Platform,
            "s_listing"
          ...
          console.log(
            "Connection created successfully between item and platform"
          );
        } catch (connectionError) {
          console.error("Error creating connection:", connectionError);
          throw new Error(
            "Failed to create connection between item and platform"
          );
        }
      } 
      setTimeout(async () => {
